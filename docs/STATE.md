...
> Última actualización: 2026-07-12 — el autor confirma el criterio de silos "por formato" (agrupamiento A–E pendiente de ver counts); sesión de prueba desde móvil (limpieza de docs).
...
**Asignación final de silos por formato.** El criterio "por formato" está **confirmado por el autor** (2026-07-12);
falta fijar el agrupamiento exacto de los 5 tipos, y para eso el autor pide ver primero los counts por tipo.

1. **[GATE — decisión del autor]** Imprimir nº de tiendas por tipo (A-E) y venta media por tipo
   (orden de escala ya conocido: A>D>B>E>C; reparto provisional por formato: Grande 27 · Mediano 8 · Pequeño 19).
   Mostrar estos counts al autor **antes** de fijar el agrupamiento. Requiere `data/raw/stores.csv`
   (no está en la VM efímera de la nube → ejecutar en local o descargar antes con `src/00_download_data.py`).
2. Reagrupar los 5 tipos en 3 formatos balanceados (el grupo "Mediano" quedó con solo 8 tiendas en la
   ...
   **Fijar el agrupamiento solo tras el visto bueno del autor sobre los counts.**
...
- ~~**Confirmar silos por formato**~~ ✅ **Criterio confirmado por el autor (2026-07-12).** Queda por decidir el
  agrupamiento exacto de los tipos A–E, que el autor fijará tras ver los counts por tipo (GATE de T1.1, arriba).
